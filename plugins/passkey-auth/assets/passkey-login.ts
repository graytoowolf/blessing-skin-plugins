...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (registerButton) {
        registerButton.addEventListener('click', async () => {
            ...
            try {
                ...
                await blessing.fetch.post(`/user/passkey/${result.data.id}/rename`, {
                    name: passKeyName
                });
                ...
                if (error instanceof DOMException && error.name === "NotAllowedError") {
                    blessing.notify.toast.error(trans('passkey-auth.verify_rejected'));
                } else {
                    blessing.notify.toast.error(trans('passkey-auth.operation_failed', { msg: (error as Error).message }));
                }
            } finally {
        ...
        button.addEventListener('click', async () => {
            ...
            if (result.value === name) {
                try {
                    ...
                    const colAuto = card.closest('.col-auto');
                    colAuto?.remove();
                } catch (error) {
    ...
    document.querySelectorAll<HTMLButtonElement>('.btn.passkey-rename').forEach(button => {
        button.addEventListener('click', async () => {
            ...
            if (result.value) {
                try {
                    await blessing.fetch.post(`/user/passkey/${id}/rename`, {
                        name: result.value
                    });
